...
class MainView(View):
    def get(self, request):
        ...
        # рандомное формирование списка из 6 туров
        list = random.sample([i for i in range(1, len(tours)+1)], 6)
        context['tours'] = {}
        for num in list:
            context['tours'][num] = tours[num]

            # замена числового значения кол-ва звезд отеля на символы звезд
            num_stars = tours[num]['stars']
            if type(num_stars) == str:
                context['tours'][num]['stars'] = [str(u'\u2605') for i in range(int(num_stars))]
...
class DepartureView(View):
    def get(self, request, departure_name):
        context = {}
        context['departure'] = departures[departure_name]
        context['tours'] = dict(filter(lambda elem: elem[1]['departure'] == departure_name, tours.items()))

        for key, value in context['tours'].items():
            num_stars = tours[key]['stars']
            if type(num_stars) == str:
                value['stars'] = [str(u'\u2605') for i in range(int(num_stars))]

        context['number'] = len(context['tours'])
        context['tour_end'] = ends['tour_end']

        # range цен и ночей
        context['min_price'] = min([context['tours'][i]['price'] for i in context['tours']])
        context['max_price'] = max([context['tours'][i]['price'] for i in context['tours']])
        context['min_nights'] = min([context['tours'][i]['nights'] for i in context['tours']])
        context['max_nights'] = max([context['tours'][i]['nights'] for i in context['tours']])
...
class TourView(View):
    def get(self, request, id):
        ...
        tour = tours[id]
        context['tour'] = tour
        context['rus_name_departure'] = departures
        context['departure'] = tour['departure']
        return render(
